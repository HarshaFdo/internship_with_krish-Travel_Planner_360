...
let CircuitBreakerService = CircuitBreakerService_1 = class CircuitBreakerService {
    constructor() {
        ...
        this.state = "CLOSED";
        this.requests = []; // Tracking last 20 requests (true=success, false=failure)
        this.halfOpenCount = 0;
        this.openedAt = 0;
    }
    async execute(serviceCall, fallback, serviceName = "service") {
        ...
        if (this.state === "OPEN") {
            ...
            if (elapsed < 30000) {
                ...
                this.halfOpenCount = 0;
                this.logger.log(`[${serviceName}] Circuit: OPEN -> HALF_OPEN (testing recovery)`);
            }
        }
        // Try to execute
        try {
            ...
            if (this.state === "HALF_OPEN") {
                ...
                if (this.requests.length >= 10) {
                    ...
                    if (failureRate >= 0.5) {
                        this.state = "OPEN";
                        this.openedAt = Date.now();
                        this.logger.error(`Circuit: CLOSED -> OPEN (${(failureRate * 100).toFixed(0)}% failures)`);
                    }
                }
            }
            this.logger.error(`[${serviceName}] Service call failed: ${error instanceof Error ? error.message : "Unknown error"}`);
            return fallback();
        }
    }
    getState() {
        ...
        return {
            state: this.state,
            failureRate: this.requests.length > 0
